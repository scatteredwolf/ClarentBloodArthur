    ...
    public class HitboxManager : MonoBehaviour
    {
        ...
        public void BakeAnimation(int animationID)
        {
            ...
            for (int k = 0; k < numFrames; k++)
            {
                ...
                _event.stringParameter = new internalFrameData(animation, k).Serialize();//JsonUtility.ToJson(new internalFrameData(animation, k));
                _event.floatParameter = animationID;
                _event.intParameter = k;
                _event.time = (clip.length / numFrames) * k;
                events.Add(_event);
                    ...
                    for (int i = 0; i < animation.framedata[k].events.Length; i++)
                        {
                            ...
                            frame_event.time = (clip.length / numFrames) * k;
            ...
            //if (animation.movementspeed != 0f && animation.movementspeed != 1f)
            {
                ...
                speed_event2.time = (clip.length / numFrames) * (numFrames - 1);
        ...
        private Vector2Int m_LastOffset;
        ...
        private void UpdateHitbox(internalFrameData animdata, int anim, int frame)
        {
            ...
            if (animdata.hasnextframe && nextframedata.smoothedOffset)
            {
                ...
                else transform.root.localPosition += new Vector3(m_OffsetStep.x * m_UPP, m_OffsetStep.y * m_UPP);
#else
        ...
        public int GetNumFrames(int animationID)
        {
            ...
            return Mathf.FloorToInt(animation.clip.length * animation.clip.frameRate);
        }
        ...
        static readonly public Color[] HitboxColor = new Color[]
    {
            new Color(0f,       1f,         1f,     0.75f), //Cyan
            new Color(1f,       0.125f,     0f,     0.75f), //Red
            new Color(0f,       0f,         1f,     0.75f), //Blue
            new Color(1f,       0.5f,       0f,     0.75f), //Orange
            new Color(1f,       1f,         0f,     0.75f), //Yellow
            new Color(0.75f,    0f,         1f,     0.75f), //Purple
            new Color(0f,       1f,         0f,     0.75f), //Green
            new Color(1f,       0f,         0.75f,  0.75f), //Pink
            new Color(0.5f,     1f,         0f,     0.75f), //Lime
            new Color(0f,       1f,         0.5f,   0.75f)  //Teal
    };
        ...
        private void OnDrawGizmos()
        {
            ...
            {
                Gizmos.color = HitboxColor[((int)collider[i].type) % HitboxColor.Length];

...
namespace BlackGardenStudios.HitboxStudioPro
{
    ...
    public class HitBoxManagerInspector : Editor
    {
        ...
        private void RepairFrameDataCount()
        {
        ...
        public void DrawEditorTimeline(Vector2 position, float width, Vector2 mouse)
        {
            ...
            {
                var frameRect = new Rect(position.x + space * i, position.y - 5, 10, 10);
                ...
                if (eventType == EventType.Repaint)
                {
                    ...
                    EditorGUI.DrawRect(new Rect(position.x + space * i + 1, position.y - 4, 8, 8),
                ...
                if (frames[i].events != null)
                {
                    ...
                    {
                        var eventRect = new Rect(position.x + space * i, position.y - 1 + 10 * (j + 1), 5, 5);
                        ...
                        if (eventType == EventType.Repaint)
                        {
                            ...
                            EditorGUI.DrawRect(new Rect(position.x + space * i + 1, position.y + 10 * (j + 1), 4, 4),
                ...
                if (eventType == EventType.Repaint && frames[i].collider != null)
                {
                    ...
                    DrawLabel(count.ToString(), new Vector2(position.x + 2f + space * i, position.y - 15), 8);
                }
            }
        ...
        public void DrawColliderRects(Vector2 position, Vector2 pivot, Vector2 pan, Vector2 mouse, float scale)
        {
            ...
            for (int i = 0; i < colliderArray.Length; i++)
            {
                ...
                var color = HitboxManager.HitboxColor[((int)collider.type) % HitboxManager.HitboxColor.Length];
        ...
        static public void DrawLabel(string text, Vector2 position, int fontSize = 12, FontStyle style = FontStyle.Normal)
        {
